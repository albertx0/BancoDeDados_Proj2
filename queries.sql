...
SELECT t.numero_temp AS temporada, t.qtd_ep AS episodios, t.nota_avaliacao AS nota
FROM "Temporada" t
...
SELECT a.nome AS ator, s.nome AS serie, p.nome_produtora
FROM "Ator" a
JOIN "Temporada" t ON a.id_ator = t.id_ator
JOIN "Serie" s ON t.id_serie = s.id_serie
JOIN "Produtora" p ON s.id_prod = p.id_prod
WHERE p.nome_produtora = 'Netflix'
ORDER BY a.nome;

--5. Séries com melhor avaliação média (considerando todas temporadas)
SELECT s.nome AS serie, AVG(t.nota_avaliacao) AS media_avaliacao
FROM "Serie" s
...
SELECT d.nome AS diretor, COUNT(s.id_serie) AS qtd_series
FROM "Diretor" d
...
JOIN "Serie" s ON t.id_serie = s.id_serie
WHERE s.nome = 'Stranger Things'
ORDER BY e.nota_avaliacao DESC
LIMIT 5;

--8. Produtoras e o total de séries que produziram
SELECT p.nome_produtora, COUNT(s.id_serie) AS total_series
FROM "Produtora" p
...
SELECT s.nome AS serie, t.numero_temp AS temporada, 
       AVG(e.duracao) AS media_duracao_minutos
FROM "Episodio" e
JOIN "Temporada" t ON e.id_temp = t.id_temp
JOIN "Serie" s ON t.id_serie = s.id_serie
WHERE s.nome = 'The Crown'
